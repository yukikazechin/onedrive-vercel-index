...
export const ChildIcon: FC<{ child: OdFolderChildren }> = ({child}) => {
    ...
    return render ? (
        ...
        <FontAwesomeIcon
            icon={child.file ? getFileIcon(child.name, {video: Boolean(child.video)}) : ['far', 'folder']}/>
    )
}
...
const FileListing: FC<{ query?: ParsedUrlQuery, renderedData?: any }> = ({query, renderedData}) => {
    ...
    if (error) {
        ...
        return (
            <PreviewContainer>
                {error.status === 401 ? <Auth authTokenPath={error.message?.authPath}/> :
    ...
    if ('folder' in responses[0]) {
        ...
        const handleSelectedDownload = () => {
            ...
            const files = getFiles()
                ...
                .map(c => ({
                    name: c.name,
                    url: `/api/raw/?path=${path}/${encodeURIComponent(c.name)}`,
                }))
        ...
        const handleSelectedPermalink = (baseUrl: string) => {
            return getFiles()
                ...
                .map(
                    c =>
                        `${baseUrl}/api/raw/?path=${path}/${encodeURIComponent(c.name)}${c?.odpt ? `&odpt=${c?.odpt}` : ''}`
                )
                .join('\n')
        }
                ...
                {(renderedData.readme?.length > 0 || readmeFile) && (
                    <div className="mt-4">
                        <MarkdownPreview file={readmeFile} path={path} standalone={false}
                                         setContent={renderedData.readme}/>
                    </div>
                )}
            </>
        )
    ...
    if ('file' in responses[0] && responses.length === 1) {
        ...
        if (previewType) {
            switch (previewType) {
                case preview.image:
                    return <ImagePreview file={file} hashedToken={file?.odpt}/>

                case preview.text:
                    return <TextPreview file={file} hashedToken={file?.odpt}/>

                case preview.code:
                    return <CodePreview file={file} hashedToken={file?.odpt}/>

                case preview.markdown:
                    return <MarkdownPreview file={file} path={path} hashedToken={file?.odpt}/>

                case preview.video:
                    return <VideoPreview file={file} hashedToken={file?.odpt}/>

                case preview.audio:
                    return <AudioPreview file={file} hashedToken={file?.odpt}/>

                case preview.pdf:
                    return <PDFPreview file={file} hashedToken={file?.odpt}/>

                case preview.office:
                    return <OfficePreview file={file} hashedToken={file?.odpt}/>

                case preview.epub:
                    return <EPUBPreview file={file} hashedToken={file?.odpt}/>

                case preview.url:
                    return <URLPreview file={file} hashedToken={file?.odpt}/>

                default:
                    return <DefaultPreview file={file} hashedToken={file?.odpt}/>
            }
        } else {
            return <DefaultPreview file={file} hashedToken={file?.odpt}/>
        }
